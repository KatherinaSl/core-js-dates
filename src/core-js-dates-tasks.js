...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.round(timeDiffrence / (1000 * 60 * 60 * 24)) + 1;
}
...
function isDateInPeriod(date, period) {
  const dates = Object.values(period);
  const startDate = new Date(dates[0]).getTime();
  const endDate = new Date(dates[1]).getTime();
  return (
    new Date(date).getTime() >= startDate && new Date(date).getTime() <= endDate
  );
}
...
function formatDate(date) {
  const newDate = new Date(date);
  const year = newDate.toDateString().split(' ')[3];
  ...
  return `${month}/${day}/${year}, ${hours}:${minutes}:${seconds} ${newTime}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  let weekends = 0;
  for (let oneDay = 1; oneDay <= daysInMonth; oneDay += 1) {
    const dayOfTheWeek = new Date(
      Date.UTC(year, month - 1, oneDay)
    ).getUTCDay();
...
function getWeekNumberByDate(date) {
  ...
  let daysDiff = (date - firstDayOfYear) / msInDay + 1;

  if (firstDayOfYear.getDay() === 0) {
    daysDiff += 6;
  } else if (firstDayOfYear.getDay() !== 1) {
    daysDiff += firstDayOfYear.getDay() - 1;
  }
...
function getNextFridayThe13th(date) {
  ...
  let next13th = new Date(currentYear, nextMonth, 13);

  while (next13th.getDay() !== 5) {
    nextMonth += 1;
    next13th = new Date(currentYear, nextMonth, 13);
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const firstDay = stringToDate(period.start);
  const lastDay = stringToDate(period.end);

  const arrayOfWorkDays = [];

  let nextDay = new Date(firstDay);

  while (nextDay <= lastDay) {
    for (let i = 0; i < countWorkDays && nextDay <= lastDay; i += 1) {
      arrayOfWorkDays.push(nextDay);
      nextDay = new Date(nextDay);
      nextDay.setDate(nextDay.getDate() + 1);
    }

    nextDay = new Date(nextDay);
    nextDay.setDate(nextDay.getDate() + countOffDays);
  }
  return arrayOfWorkDays.map(
    (day) =>
